...
class Hopenet_mxJiang_pitch(HybridBlock):
    def __init__(self, num_bins, **kwargs):
        ctx = kwargs.pop('ctx')
        super(Hopenet_mxJiang_pitch, self).__init__(**kwargs)
...
class Hopenet_mxJiang_extra(HybridBlock):
    def __init__(self, num_bins, **kwargs):
        ...
        self.model_pitch = nn.HybridSequential(prefix='pitch')
        with self.model_pitch.name_scope():
            self.model_pitch.add(mx.gluon.nn.Dense(num_bins))
            
    def hybrid_forward(self, F, x):  
        out_model_pitch = self.model_pitch(x)
        return out_model_pitch   
