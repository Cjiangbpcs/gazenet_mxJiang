...
class Hopenet_mxJiang_yaw(HybridBlock):
    def __init__(self, num_bins, **kwargs):
        ctx = kwargs.pop('ctx')
        super(Hopenet_mxJiang_yaw, self).__init__(**kwargs)
...
class Hopenet_mxJiang_extra(HybridBlock):
    def __init__(self, num_bins, **kwargs):
        ...
        self.model_yaw = nn.HybridSequential(prefix='yaw')
        with self.model_yaw.name_scope():
            self.model_yaw.add(mx.gluon.nn.Dense(num_bins))     
            
    def hybrid_forward(self, F, x):  
        out_model_yaw = self.model_yaw(x)  
        return out_model_yaw 
